...
import { checkApiHealth, login } from "@/lib/api";
        ...
        <div className="mx-auto w-full max-w-md">
          ...
          {apiOnline === false && (
            <div className="mb-4 rounded-xl border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-800 dark:bg-amber-950/30 dark:text-amber-200">
              Backend API is offline. Start it from <code className="text-xs">apps/api</code>.
            </div>
          )}
...
export async function logout(role?: Role) {
  try {
    await fetch(
      typeof window !== "undefined" ? "/backend/api/v1/auth/logout" : "http://127.0.0.1:8000/api/v1/auth/logout",
